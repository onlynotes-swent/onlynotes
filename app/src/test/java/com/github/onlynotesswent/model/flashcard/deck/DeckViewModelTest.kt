...
class DeckViewModelTest {
    ...
    private val testDeck:Deck = Deck(
        id = "1",
        name = "Deck",
        userId = "2",
        folderId = "3",
    ...
    fun `getDecksFrom fails`(){
        ...
        deckViewModel.getDecksFrom(testDeck.userId, {fail("Should not succeed")}, {exceptionThrown=it})
        verify(mockDeckRepository).getDecksFrom(eq(testDeck.userId), any(), any())
    ...
    fun `getDecksByFolder calls repository`() {
        var wasCalled = false
        deckViewModel.getDecksByFolder(testDeck.folderId!!, {wasCalled=true}, {fail("Should not fail")})
    ...
    fun `getDecksByFolder fails`(){
        ...
        deckViewModel.getDecksByFolder(testDeck.folderId!!, {fail("Should not succeed")}, {exceptionThrown=it})
        verify(mockDeckRepository).getDecksByFolder(eq(testDeck.folderId), any(), any())
        assertEquals(testException, exceptionThrown)
    }
