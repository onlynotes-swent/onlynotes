...
class DeckRepositoryFirestoreTest {
  ...
  fun setUp() {
    ...
    `when`(mockCollectionReference.whereEqualTo("folderId", testDeck.folderId))
        .thenReturn(mockQuery)
    `when`(mockCollectionReference.whereEqualTo("userId", testDeck.userId)).thenReturn(mockQuery)
  ...
  fun testAddFlashcardId() {
    var wasCalled = false
    deckRepository.addFlashcardToDeck(
        testDeck.id, testFlashcardId, { wasCalled = true }, { fail("Should not fail") })
  ...
  fun testAddFlashcardIdFailure() {
    ...
    var exception: Exception? = null
    deckRepository.addFlashcardToDeck(
        testDeck.id, testFlashcardId, { fail("Should not succeed") }, { exception = it })
    verify(mockDocumentReference).update(eq("flashcardIds"), any())
  ...
  fun testAddFlashcardIds() {
    var wasCalled = false
    deckRepository.addFlashcardsToDeck(
        testDeck.id, testFlashcardIds, { wasCalled = true }, { fail("Should not fail") })
    verify(mockDocumentReference).update(eq("flashcardIds"), any())
    assert(wasCalled)
  }
  ...
  fun testAddFlashcardIdsFailure() {
    ...
    var exception: Exception? = null
    deckRepository.addFlashcardsToDeck(
        testDeck.id, testFlashcardIds, { fail("Should not succeed") }, { exception = it })
    verify(mockDocumentReference).update(eq("flashcardIds"), any())
  ...
  fun testRemoveFlashcard() {
    var wasCalled = false
    deckRepository.removeFlashcardFromDeck(
        testDeck.id, testFlashcardId, { wasCalled = true }, { fail("Should not fail") })
    verify(mockDocumentReference).update(eq("flashcardIds"), any())
    assert(wasCalled)
  }
