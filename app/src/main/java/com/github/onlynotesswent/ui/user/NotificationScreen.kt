            ...
            val notification = userNotifications.value[index];
              //for each notifcationType do something:
                when (notification.type) {
                    Notification.NotificationType.FOLLOW_REQUEST -> NotificationTypeFollowRequest(notification,fileViewModel,userViewModel,notificationViewModel)
...
@Composable
fun NotificationTypeFollowRequest(notification: Notification, fileViewModel: FileViewModel,userViewModel: UserViewModel,notificationViewModel: NotificationViewModel) {
    Box(modifier = Modifier.testTag("notification-${notification.id}").padding(8.dp).alpha(if (notification.read) 0.5f else 1f)) {
    Column {
        Text( notification.title,  style = Typography.titleMedium)
       Row {
           assert(notification.senderId != null) //the senderId should not be null for this type of notification
           ...
           if (!notification.read) {
               Button(onClick = {
                   ...
                   userViewModel.getUserById(notification.senderId, { user ->
                       notificationViewModel.updateNotification(notification.copy(read = true, type = Notification.NotificationType.FOLLOW_REQUEST_ACCEPTED, body = user.userHandle() + " is now following you"))
                   }, {
                   }, {
                   })
               }) {
               ...
               Button(onClick = {
                   ...
                   userViewModel.getUserById(notification.senderId, { user ->
                          notificationViewModel.updateNotification(notification.copy(read = true, type = Notification.NotificationType.FOLLOW_REQUEST_REJECTED, body = "you have rejected the request from:" +user.userHandle()  ))
                   }, {
                   }, {
                   })
               }){
